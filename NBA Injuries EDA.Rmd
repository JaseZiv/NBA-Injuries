...
The following analysis has been conducted using Randall Hopkins' data set found here https://www.kaggle.com/ghopkins/nba-injuries-2010-2018 and covers the period between 2010 and 2018. The data was sourced by scraping the Pro Sports Transactions website (prosportstransactions.com).
...
Only 470 events were coded as `rest`. That's only `r paste0(round(mean(injuries$rest == TRUE)*100,2),"%")`. Still, it was enough for the NBA top brass to question teams over their handling of players in games with little meaning heading in to the Playoffs (they mean you Coach Pop). Teams were resting players and it was said to be bad for business - fans come out to watch their stars on the court, not in street clothes. Additionally, teams may also rest players to increase their chances of finishing lower and having more balls in the lottery.
...
Hmmm looks like Pop does rest his players the most - NBA management be damned!
...
rest_by_team_year %>%
  ...
  scale_x_continuous(  # This handles replacement of row 
      breaks = rest_by_team_year$row, # notice need to reuse data frame
      labels = rest_by_team_year$Team) +
  coord_flip() +
  labs(x= "Team", y= "Number Rested", title = "Which teams are the biggest culprits?") +
...
Would welcome any feedback at all!
...
removed_words <- c("to", "from", "on", "in")

count_bigrams <- function(dataset) {
  dataset %>%
    unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
    separate(bigram, c("word1", "word2"), sep = " ") %>%
    filter(!word1 %in% removed_words,
           !word2 %in% removed_words) %>%
...
injuries <- injuries %>% rename(text = cause)

injuryWords <- injuries %>%
  count_bigrams()

injuryWords %>%
  filter(!is.na(word1), n > 30) %>%
  visualize_bigrams()
